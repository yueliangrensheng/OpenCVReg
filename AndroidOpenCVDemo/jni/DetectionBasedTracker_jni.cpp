#include <com_diwen_android_opencv_DetectionBasedTracker.h>
...
public:
	...
	void detect(const cv::Mat &Image, std::vector<cv::Rect> &objects) {
		...
		LOGD(
				"CascadeDetectorAdapter::Detect: scaleFactor=%.2f, minNeighbours=%d, minObjSize=(%dx%d), maxObjSize=(%dx%d)", scaleFactor, minNeighbours, minObjSize.width, minObjSize.height, maxObjSize.width, maxObjSize.height);
...
JNIEXPORT jlong JNICALL Java_com_diwen_android_opencv_DetectionBasedTracker_nativeCreateObject(
	...
	try {
		cv::Ptr<CascadeDetectorAdapter> mainDetector = makePtr<
				CascadeDetectorAdapter>(
				makePtr<CascadeClassifier>(stdFileName));
		cv::Ptr<CascadeDetectorAdapter> trackingDetector = makePtr<
				CascadeDetectorAdapter>(
				makePtr<CascadeClassifier>(stdFileName));
...
JNIEXPORT void JNICALL Java_com_diwen_android_opencv_DetectionBasedTracker_nativeSetFaceSize(
	...
	LOGD(
			"Java_org_opencv_samples_facedetect_DetectionBasedTracker_nativeSetFaceSize -- BEGIN");
	...
	LOGD(
			"Java_org_opencv_samples_facedetect_DetectionBasedTracker_nativeSetFaceSize -- END");
}
...
JNIEXPORT void JNICALL Java_com_diwen_android_opencv_DetectionBasedTracker_nativeDetect(
	...
	LOGD(
			"Java_org_opencv_samples_facedetect_DetectionBasedTracker_nativeDetect END");
}
